...
procedure Lape_Overlay_SetMousePath(const Params: PParamArray); cdecl;
begin
  PWindowOverlay(Params^[0])^.DrawMousePath := PBoolean(Params^[1])^;
end;
...
begin
  case Index of
    ...
    9:
      begin
        Addr := @Lape_Overlay_SetMousePath;
        StrPCopy(Decl, 'procedure TWindowOverlay.SetMousePath(Toggle: Boolean); native;');
      end;
    10:
      begin
